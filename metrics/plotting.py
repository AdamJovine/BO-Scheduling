...
def plot_pairwise():
    ...
    for col_x, col_y in itertools.combinations(columns, 2):
        if pd.api.types.is_numeric_dtype(df[col_x]) and pd.api.types.is_numeric_dtype(df[col_y]):
            ...
            plt.plot(frontier_points[col_x], frontier_points[col_y],
            ...
            fname = f"{model}plot_{col_x}_{col_y}.png".replace(' ', '_').replace(',', '').replace('/', '_')
...
def hypervolume():
    ...
    ref_point  = np.array([1, 10, 30, 30, 100, 100, 1000, 2000,
                           2000, 5000, 2000, 2000, 20, 60000], dtype=float)
    objectives = [
    ...
    for ts, path in timed_csv:
        ...
        front = np.array([
            p for p in all_pts
            if not any((np.all(q <= p) and np.any(q < p)) for q in all_pts if not np.array_equal(q, p))
        ])
        if front.size == 0:
            continue
        ...
        pct_gain = 0.0 if prev_norm in (None, 0) else (norm_hv - prev_norm) / prev_norm * 100
        prev_norm = norm_hv
        ...
        print(f"{ts} → rawHV={raw_hv:.3e}, normHV={norm_hv:.3e}, "
...
def combined_graph():
    ...
    if not csv_files:
        raise FileNotFoundError("No 'hypervo_progression_*.csv' files found in the current directory.")

    hv_data = {}
    for path in csv_files:
        df = pd.read_csv(path)
        model = os.path.basename(path).replace("hypervolume_progression_", "").replace(".csv", "")
        ...
        if 'norm_hv' in df.columns:
            ...
            raise KeyError(f"No normalized hypervolume column found in {path}. Available columns: {df.columns.tolist()}")
...
# Assuming utils.helpers is correctly located relative to the script
# If not, adjust the path insertion accordingly
try:
    # Try finding the script's directory if __file__ is defined
    ...
    print(f"Warning: __file__ not defined. Assuming package root is: {package_root}")

if package_root not in sys.path:
    sys.path.insert(0, package_root)
    print(f"Added to sys.path: {package_root}")
...
def plot_best_custom_utility():
    ...
    if not timed_csv_files:
        print(f"No CSV files found for model '{model}' within the specified time range.")
        return
    ...
    for i, (ts, path) in enumerate(timed_csv_files):
        ...
        if missing_cols:
            print(f"  Skipping file {os.path.basename(path)} due to missing columns: {missing_cols}")
            continue
        ...
        try:
            ...
            if valid_rows.empty:
                 print(f"  Skipping file {os.path.basename(path)} as it has no valid (non-NaN) objective rows.")
                 continue
            Y = torch.tensor(valid_rows[objective_cols].values, dtype=torch.float)
        except Exception as e:
            print(f"  Error converting data to tensor in {os.path.basename(path)}: {e}. Skipping.")
            continue

        # Apply custom utility function
        if Y.numel() == 0: # Check if tensor is empty after potential filtering
            print(f"  Skipping file {os.path.basename(path)} as objective tensor is empty.")
            continue

        try:
            utilities = custom_utility(Y)
            if utilities.numel() == 0:
                 print(f"  Skipping file {os.path.basename(path)} as utility calculation resulted in empty tensor.")
                 continue
            max_util = utilities.max().item()
        except Exception as e:
             print(f"  Error calculating utility for {os.path.basename(path)}: {e}. Skipping.")
             continue
    ...
    plt.plot_date(mdates.date2num(timestamps), best_utils, linestyle='-', marker='o', markersize=4, label=f"Best Utility ('{model}')")

    plt.title(f"Best Custom Utility Progression ('{model}')\n({start_ts.strftime('%Y-%m-%d %H:%M')} to {end_ts.strftime('%Y-%m-%d %H:%M')})")
    ...
    plt.gca().xaxis.set_major_locator(plt.MaxNLocator(10)) # Limit number of major ticks
...
def plot_best_custom_utility_by_iteration():
    ...
    for model in models_to_compare:
        ...
        for i, path in enumerate(filtered_files):
            ...
            try:
                utilities = custom_utility(Y)
                max_util = utilities.item() if utilities.ndim == 0 else utilities.max().item()
            except Exception as e:
        ...
        print(f"  {model}: {len(best_utils)} iterations processed, final best = {best_so_far:.4f}")
    ...
    for model, data in all_results.items():
        if not data['iters']:
            continue
        plt.plot(data['iters'], data['best_utils'], label=model, color=colors[model], marker='o')
    ...
    plt.tight_layout()
    save_path = os.path.join(output_dir, "best_custom_utility_comparison_by_iteration.png")
    plt.savefig(save_path)
    print(f"\nPlot saved to: {save_path}")
    plt.show()

plot_best_custom_utility_by_iteration()
...
def find_better(): 
    ...
    # === DISPLAY RESULTS ===
    results = sorted(results, key=lambda x: -x["num_lower_metrics"])  # Sort by best dominance
    for res in results:
        print(f"{res['file']} | Lower in {res['num_lower_metrics']} metrics | Values: {res['values']}")
#find_better()
...
def pca():
    ...
    for label in label_keywords.values():
        ...
        plt.scatter(X_pca[idx, 0], X_pca[idx, 1], label=label, color=colors[label], alpha=0.7)
