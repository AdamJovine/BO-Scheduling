import pandas as pd
import numpy as np
from config.settings import PARAM_NAMES, SAVE_PATH, BLOCK_BOUNDS, SEQ_BOUNDS , PP_BOUNDS, DATA_PATH
from sqlalchemy import create_engine, text
from myproject.data.db_accessor import add_metric 
...
def compute_lateness(schedule_df, exam_sizes):
    ...
    schedule_df['weighted'] = schedule_df['lateness'] * schedule_df['size'].apply(lambda x: 0 if x < 100 else x)
...
def compute_conflicts(by_student_block):
    ...
    for s in range(len(blocks_copy)):
        for b in range(len(blocks_copy['slots'][s])):
            if blocks_copy['slots'][s][b] != -1 and blocks_copy['slots'][s].count(blocks_copy['slots'][s][b]) > 1:
                conflicts += blocks_copy['slots'][s].count(blocks_copy['slots'][s][b]) - 1
...
def evaluate_schedule(schedule, exam_sizes, params, global_sets, sched_name, slots_per_day=3):
    """
